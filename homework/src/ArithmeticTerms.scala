  ...
  def unify(c: List[Equation], n: Int): Option[List[Equation]] = {
    ...
    case Equation(s, t) :: tail =>
        (s, t) match {
          ...
          case (x: ArithmeticVariable, t: ArithmeticTerm) if !getVars(t).contains(x) & (tail == Nil | tail.foldLeft(false)((ans, eq) => getVars(eq).contains(x) | ans)) =>
            val pp = arithm_subst(tail, x, t)
            unify(pp, 0) match {
              case Some(ans) => Some(ans ::: List(Equation(x, t)))
              case None => None
            }
          ...
          case (Function(s_name, s_args), Function(t_name, t_args)) => if (s_args.size != t_args.size | !s_name.equals(t_name) ) None else {
            unify(tail ::: s_args.zip(t_args).map(x => Equation(x._1, x._2)), 0)
          }
