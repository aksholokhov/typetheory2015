...
import sholokhov.lambdas.parser.{Variable, NonTypedLambdaParser}
...
object Task2 {
  def main(args: Array[String]) {
    ...
    val out = new PrintWriter("tests/task2/output")
    val str = Source.fromFile("tests/task2/test1.in").getLines().map(_.toString.replace(" ", "_"))
